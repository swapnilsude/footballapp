...
class FootballApp(object):
	def __init__(self, league, team, results, fixture):
		super(FootballApp, self).__init__()
		self.league = league
		self.team = team
	...
	def get_results_fixture_team(self):
		...
		if not self.fixture and not self.results:
			self.get_results_fixture_team_aux(soup, False)
	...
	def get_league_table(self):
		...
		df = pd.DataFrame (print_list,columns=['Rank','Team','Points','Played','Won','Draw','Lost','GF','GA','GD'])
...
def main(args):
	...
	footballapp = FootballApp(league, team, results, fixture)
	footballapp.get_data()
